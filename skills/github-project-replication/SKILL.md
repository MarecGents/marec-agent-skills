...
复现项目的第一步是获取目标仓库的代码。请严格按照以下优先级依次尝试，直到成功获取代码。此规范确保在各种网络环境下都能有效地获取目标仓库内容。
...
**获取目标仓库代码** — 根据本规范的「GitHub 仓库获取规范」章节，按照优先级依次尝试获取源码。将获取过程和结果记录到 `progress.md`。
...
| **无法获取仓库代码** | 按「GitHub 仓库获取规范」的优先级依次尝试，所有方式均失败后向用户报告具体情况并请求协助 |
